...
let context = util.context("war3event", {
...
function activateUnitEvent<C extends AnyEventCallback>(e: War3Event<C>) {
    let eventId = (e as any)._warEventId
    if (context.activatedHandlers.has(eventId)) {
        return
    }

    let trigger = CreateTrigger()
    let collector = context.collectors.get(eventId)! as Collector<C>
...
export namespace U {
    ...
    function dispatchTarget(): Unit | Item | Destructable {
        ...
        let destructable = GetSpellTargetDestructable()
...

{
    ...
    const ABILITY_ID = compiletime(() => {
        ...
        let abilityId = util.idgen.abil()

        if (currentMap != undefined) {
            let defendClone = (currentMap.objects.ability["Adef"] as WarObject).clone()
            defendClone.acat = ""
            defendClone.aart = ""
...
let chatEvents: jtrigger[] = util.context("chatevent", [])
